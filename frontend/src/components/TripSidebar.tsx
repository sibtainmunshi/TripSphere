...
export function TripSidebar({ mobileOpen = false, onClose }: TripSidebarProps = {}) {
  ...
  // Same "which TripMember am I" lookup ChatPage uses to attribute messages —
  // needed here too so a trip's own sent messages never count as unread.
  const myMemberId = trip?.members?.find((member) => member.email === user?.email)?.id
  const chatHasUnread = useUnreadChat(tripId, myMemberId)
    ...
    >
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {module.label}
                {module.path === 'chat' && chatHasUnread && !isActive && (
                  <span className="ml-auto h-2 w-2 shrink-0 rounded-full bg-ocean" aria-hidden="true" />
                )}
              </>
            )}
          </NavLink>
      ...
      <div className="scrollbar-none mt-5 flex-1 overflow-y-auto px-3">
        <div className="flex items-center justify-between px-3.5 pb-1">
          ...
          <NavLink to="/trips" onClick={onClose} aria-label="Create new trip" className="text-slate hover:text-ink">
